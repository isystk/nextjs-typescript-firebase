...
import { readConst, registConfirm } from '@/actions'
...
export class EntryRegist extends React.Component<IProps, IState> {
  ...
  async onSubmit(values): Promise<void> {
    ...
    Router.push(URL.ENTRY_REGIST_CONFIRM)
  }
                ...
                >
                  ...
                  {this.props.consts.prefecture &&
                    _.map(this.props.consts.prefecture.data, (e, index) => (
                      <option value={e.code} key={`prefecture${index}`}>
                        {e.text}
                      </option>
                    ))}
                </Field>
...
const validate = (values) => {
  ...
  if (!values.familyNameKana)
    errors.familyNameKana = 'お名前カナ(セイ)を入力して下さい'
  ...
  if (!values.passwordConf)
    errors.passwordConf = 'パスワード確認を入力して下さい'
  if (values.password !== values.passwordConf)
    errors.passwordConf = '入力されたパスワードが一致しません'
...
export default connect(
  ...
  reduxForm({ validate, form: 'entryRegistForm', enableReinitialize: true })(
    EntryRegist
  )
)
